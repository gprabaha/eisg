...
psth_ps = compose( {'mua_psth/%d_ms_eye_events', 'mua_psth/%d_ms_dot_events'}, desired_ms );

if ( is_per_stim )
  psth_ps = fullfile( psth_ps, 'per_stim' );
else
  psth_ps = fullfile( psth_ps, 'all_rois' );
end
...
if ( do_nmatch )
  n_I = findeach( tot_tbl, {'session', 'psth_type'} );
  keep_label = false( size(tot_tbl, 1), 1 );
  for i = 1:numel(n_I)
    t_I = findeach( tot_tbl, 'hemifield_label', n_I{i} );
    min_n = min( cellfun(@numel, t_I) );
%     select_trials = @(x) sort(randsample(numel(x), min_n));
    select_trials = @(x) 1:min_n;
    t_I = cellfun( @(x) x(select_trials(x)), t_I, 'un', 0 );
    keep_label(vertcat(t_I{:})) = true;
  end
  tot_tbl.hemifield_label = compose( "%s-nmatch", tot_tbl.hemifield_label );
  tot_tbl.hemifield_label(~keep_label) = "";
end
...
%   mask = mask & tot_tbl.hemifield_label == 'ipsi';
  mask = mask & tot_tbl.hemifield_label ~= "";
...
plt_mask = model_C.monkey == 'tarantino';
plt_mask(:) = true;

% is_sig = perm_term_ps < 0.05 & term_ps < 0.05;
is_sig = perm_term_ps < 0.05;
[I, id, C] = rowsets( 3, model_C, panels, 'stim_type', 'region' ...
  , 'to_string', true, 'mask', plt_mask );
...
term_vs = terms;
% term_vs = cellfun( @(x) x.Rsquared.Adjusted, lms );

% plt_mask = abs( term_vs ) < lms

% plt_mask = model_C.monkey == 'lynch' & model_C.psth_type == 'eyes';
plt_mask = model_C.psth_type == 'dot';
% plt_mask(:) = true;

pcats = {'psth_type', 'region'};
gcats = {};
if ( per_hf_label )
%   pcats{end+1} = 'hemifield_label';
  gcats{end+1} = 'hemifield_label';
end

[I, id, C] = rowsets( 4, model_C, pcats, {'stim_type'}, gcats, 'session' ...
  , 'to_string', true ...
  , 'mask', plt_mask ...
);
...
% mask = intersect( mask, find(tot_tbl.psth_type == 'eyes') );
...
first_look_only = false;
% fname = 'p_sig_stim_v_sham_first_look_eyes_only';
fname = 'p_sig_stim_v_sham_looks_0.2__1.5_all_rois';
...
if ( 0 )
  mask = mask & (tot_tbl.roi == 'face' | tot_tbl.roi == 'eyes_nf');
...
if ( first_look_only )
  [~, ~, ic] = unique( [string(tot_tbl.stim_time), tot_tbl.session], 'rows' );
  gi = groupi( ic );
  ok_stims = nan( numel(gi), 1 );
  for i = 1:numel(gi)
    % first event after stim
    t_stim = time_to_stim(gi{i});
    [~, mi] = min( t_stim );
    ok_stims(i) = gi{i}(mi);
  end

  mask = intersect( mask, ok_stims );
end
...
[reg_I, id, C] = rowsets( 2, stim_C, {'region', 'psth_type'}, {'monkey'} ...
  , 'to_string', true );
...
if ( 1 )
  save_p = fullfile( data_root, 'plots', '082523', sprintf('%s.fig', fname) );
  saveas( gcf, save_p );
end

%  auc percent significant, in time window, data from above

by_monk = true;

props = double( is_sig );
props = props(:, psth_t >= 0 & psth_t <= 1.5);

auc_each = {'region', 'psth_type'};
if ( by_monk )
  auc_each{end+1} = 'monkey';
end

[I, C] = findeach( stim_C, auc_each );
mus = cate1( eachcell(@(x) nanmean(props(x, :), 1), I) );
psig_auc = trapz( mus, 2 );

if ( by_monk )
  [I, id, L] = rowsets( 3, C, 'region', 'psth_type', 'monkey', 'to_string', true );
else
  [I, id, L] = rowsets( 3, C, {}, 'psth_type', 'region', 'to_string', true );
end

figure(1); clf;
axs = plots.simplest_barsets( psig_auc, I, id, L );
shared_utils.plot.match_ylims( axs );
ylabel( axs(1), 'auc of proportion of significant sites over time, [0, 1.5]s (au)' );

if ( 1 )
  saveas( gcf, strrep(save_p, 'p_sig_stim_v_sham_looks', 'auc') );
end
...
[reg_I, id, C] = rowsets( 3, stim_C, {'region'}, {'monkey'}, {'psth_type'} ...
  , 'to_string', true );
