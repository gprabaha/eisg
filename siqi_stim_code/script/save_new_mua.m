...
if ( ~use_fix_events )
  mua_psth_p = fullfile( mua_psth_p, 'per_stim' );
elseif ( isempty(want_rois) )
...
for i = 1:numel(unsorted_fs)
  ...
  event_stop_ts = evt_start_ts + 0.1; % arbitrary for the case that we don't align to fix events
  ...
  if ( use_fix_events )
    ...
    for j = 1:numel(evt_ind)      
      ...
      if ( isfield(roi_file, 'm2') )
        ...
        [roi_p, sesh_fname] = fileparts( ps{evt_ind(j), 4} );        
        poss_fnames = contains( ps(:, 4), sesh_fname(1:8) ) & contains( ps(:, 4), 'position_' );
        [~, poss_names] = cellfun( @fileparts, ps(poss_fnames, 4), 'un', 0 );
        poss_nums = cellfun( @(x) str2double(x(19:end)), poss_names );
        assert( ~any(isnan(poss_nums)) );
        [~, first_num] = min( poss_nums );
        
        pos_fname = sprintf( '%s.mat', poss_names{first_num} );
        warning( 'Using social-task (%s) roi file for non-social control eye roi (%s)', pos_fname, sesh_fname );
        
        m2_roi_file = shared_utils.io.fload( fullfile(roi_p, pos_fname) );        
        m2_eyes_roi = m2_roi_file.m2.rects('eyes_nf');
        
        off_p = fileparts( ps{evt_ind(j), 5} );
        offsets_file = shared_utils.io.fload( fullfile(off_p, pos_fname) );
      end
  ...
  for idx = 1:numel(evt_subset_I)
    ...
    if ( use_fix_events )
      ...
      curr_eye_dists = nan( size(evt_stops) );
      curr_other_dists = nan( size(evt_stops) );
      curr_hemi_dists = nan( size(evt_stops) );
    end
  ...
  %   [psth, psth_t] = bfw.event_psth( evts, {kept_ts}, evt_I, spk_I, -0.1, 5, 0.1 );
    ...
    if ( use_fix_events )
      ...
      base_tbl.event_time = base_tbl.stim_time;
      base_tbl.event_duration = evt_dur;
      base_tbl.psth = psth{1};
      base_tbl.psth_t = repmat( psth_t, size(psth{1}, 1), 1 );
