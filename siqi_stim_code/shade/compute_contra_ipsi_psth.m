function [allpsth, psth_ts, all_spike_counts_in_fixation_interval, index_of_fixations] = ...
    compute_contra_ipsi_psth(spike_file, sessions, contra_ipsi_mask, fixation_start_ts, fixation_stop_ts, include_psth_timecourse)

assert( numel(contra_ipsi_mask) == numel(sessions) );
assert( numel(sessions) == numel(fixation_start_ts) );
assert( numel(fixation_start_ts) == numel(fixation_stop_ts) );

allpsth = cell( numel(spike_file.spike_times), 1 );
all_spike_counts_in_fixation_interval = cell( size(allpsth) );
psth_ts = cell( size(allpsth) );
index_of_fixations = cell( size(allpsth) );
