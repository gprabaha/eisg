...
disp('Loading data...');
data_p = fullfile( eisg.util.project_path, 'processed_data'); % Data path
% Sorted unit firing times
...
% Add face and object ROIs to behavioral data labels
events = add_whole_face_whole_object_rois( events );
% Find the start time of each event
evts = bfw.event_column( events, 'start_time' );
% Labels for the celltype of units
ct_labels = load_cell_type_labels( data_p );
% Convert spiking data struct to 2D fcat matrix
[unit_spike_ts, unit_wfs, spike_labels] = linearize_sorted( sorted );
bfw.add_monk_labels( spike_labels ); % Participating monkeys
% Find indices all units and the corresponding UUID
[uuid_I, uuids] = findall( spike_labels, 'uuid', find(spike_labels, {'valid-unit', 'maybe-valid-unit'}) );
% Find the indices in ct_labels matching each UUID
match_I = bfw.find_combinations( ct_labels, uuids );
% For each UUID find the corresponding indices in ct_labels and add the
% labels to spike_labels for the PSTH matrix
...
%% Extract PSTH corresponding to fixations into ROIs
% Declare labels of each relevant ROI
rois = { 'face', 'eyes_nf', 'whole_face', 'right_nonsocial_object_whole_face_matched' };
% Only include m1 initiated events of fixations into ROIs of interest
evt_mask = find( events.labels, [{'m1'}, rois] );
% Only include spikes from valid and maybe-valid units
...
[cell_type_I, cell_types] = findall( anova_labels, {'cell-type', 'region'}, findnone(anova_labels, {'outlier', 'dmpfc', 'ofc'}) );
cell_type_props = cellfun( @(x) pnz(sig_cells(x)), cell_type_I );

prop_table = table( cell_type_props, 'RowNames', fcat.strjoin(cell_types, '_') );
prop_table

%%  extract cell ids for hanova

anova_cell_ids = combs( anova_labels, 'uuid', find(sig_cells) );

%%

chi2_mask = findnone( anova_labels, 'outlier' );
...
cell_type_count = cellfun( @(x) numel(sig_cells(x)), cell_type_I );
prop_table = table( cell_type_count, 'RowNames', fcat.strjoin(cell_types, '_') );
prop_table

[tbl, reg_labels] = do_prop_tests( sig_cells, use_labels', findnone(use_labels,  {'outlier', 'ofc', 'dmpfc' }) );
tbl

%%
cell_type_props = cellfun( @(x) sum(sig_cells(x) & post_rs_zs(x) > 0), cell_type_I );
...
[tbl, reg_labels] = do_prop_tests( post_face_high_cells( sig_cells ), use_labels', findnone(use_labels, {'outlier', 'ofc', 'dmpfc' }) );
tbl
...
[count_labels, cell_type_I, cell_types] = keepeach( use_labels', {'cell-type', 'region'}, findnone(use_labels, {'outlier', 'ofc', 'dmpfc' }) );
...
[tbl, reg_labels] = do_prop_tests( sig_eye_cells, use_labels', fifndnone(use_labels, {'outlier', 'ofc', 'dmpfc' }) );
tbl
%%
...
% [cell_type_I, cell_types] = findall( pre_rs_outs.rs_labels, {'cell-type', 'region'}, findnone(rs_labels, 'b') );
[cell_type_I, cell_types] = findall( pre_rs_outs.rs_labels, {'cell-type', 'region'}, findnone(rs_labels, 'outlier') );
cell_type_props = cellfun( @(x) sum(sig_cells(x) & post_rs_zs(x) > 0) / sum(sig_cells(x)), cell_type_I );
